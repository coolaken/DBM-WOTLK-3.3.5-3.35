...
local specWarnPoisonNova		= mod:NewSpecialWarningRun(68989, "Melee", nil, nil, 4, 2)
...
--local soundPoisonNova					= mod:NewAnnounce(68989, nil, nil, "Melee")
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(68989, 70434) then				-- Poison Nova
		...
		specWarnPoisonNova:Play("runaway")
	end
end
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg)
	if msg == L.Barrage or msg:find(L.Barrage) then
		specWarnMines:Show()
		DBM:PlaySoundFile("Interface\\AddOns\\DBM-VPYike\\keepmove.ogg")
	end
end
