...
DBM = {
	Revision = parseCurseDate("20210402041541"),
	DisplayVersion = "3.35.11 alpha", -- the string that is shown as version
	ReleaseRevision = releaseDate(2021, 4, 2) -- the date of the latest stable version that is available, optionally pass hours, minutes, and seconds for multiple releases in one day
}
...
elseif LOCALE_zhCN then
	standardFont = "Fonts\\ZYKai_T.TTF"
...
local UnitIsGroupLeader, UnitIsGroupAssistant = UnitIsPartyLeader, IsRaidOfficer
...
local function checkForSafeSender(sender, checkFriends, checkGuild, filterRaid)
	...
	if checkGuild then
		local scanTotal =GetNumGuildMembers()--Attempt CPU saving, if "show offline" is unchecked, we can reliably scan only online members instead of whole roster
...
local function sendSync(prefix, msg)
	...
	if zoneType == "pvp" or zoneType == "arena" then
		SendAddonMessage("D4C", prefix .. "\t" .. msg, "BATTLEGROUND")
...
do
	...
	function DBM:ADDON_LOADED(modname)
		if modname == "DBM-Core" and not isLoaded then
			...
			onLoadCallbacks = nil
			loadOptions(self)
			if IsAddOnLoaded("VEM-Core") then
				self:Disable(true)
			...
			if IsAddOnLoaded("DBM-RaidLeadTools") and not self.Options.DebugMode then
				self:Disable(true)
				self:Schedule(15, infniteLoopNotice, self, L.OUTDATEDRLT)
				return
			end
			if IsAddOnLoaded("DPMCore") then
				self:Disable(true)
				C_TimerAfter(15, function() AddMsg(self, L.DPMCORE) end)
				return
			end
			if IsAddOnLoaded("DBM-VictorySound") then
				self:Disable(true)
				C_TimerAfter(15, function() AddMsg(self, L.VICTORYSOUND) end)
				return
			end
			if IsAddOnLoaded("DBM-LDB") then
				...
				if GetAddOnMetadata(i, "X-DBM-Mod") then
					if enabled ~= 0 then
						if checkEntry(bannedMods, addonName) then
							...
							local mapIdTable = {strsplit(",", GetAddOnMetadata(i, "X-DBM-Mod-LoadZoneID") or "")}
							tinsert(self.AddOns, {
...
function DBM:LFG_PROPOSAL_SHOW()
	if self.Options.ShowQueuePop and not self.Options.DontShowBossTimers then
		...
		fireEvent("DBM_TimerStart", "DBMLFGTimer", L.LFG_INVITE, 40, "71328", "extratimer", nil, 0)
	end
...
do
	...
	local function Pull(timer)
		if (DBM:GetRaidRank(playerName) == 0 and IsInGroup()) or select(2, IsInInstance()) == "pvp" or (timer > 0 and timer < 3) then
	...
	local function Break(timer)
		if IsInGroup() and DBM:GetRaidRank(playerName) == 0 or select(2, IsInInstance()) == "pvp" then--No break timers if not assistant or if it's dungeon/raid finder/BG
	...
	SlashCmdList["DEADLYBOSSMODS"] = function(msg)
		...
		elseif cmd:sub(1, 8) == "whereiam" or cmd:sub(1, 8) == "whereami" then
			if DBM:HasMapRestrictions() then
				local map = GetInstanceInfo("player")
				...
				--MK
				local x, y, _, map = GetInstanceInfo("player")
...
do
	local function updateRangeFrame(r, reverse)
		local hrs = DBM:HasMapRestrictions()
...
--MK
		...
		accept:SetNormalTexture("Interface\\Buttons\\UI-DialogBox-Button-Up")--"Interface\\Buttons\\UI-DialogBox-Button-Up"
		accept:SetPushedTexture("Interface\\Buttons\\UI-DialogBox-Button-Down")--"Interface\\Buttons\\UI-DialogBox-Button-Down"
		accept:SetHighlightTexture("Interface\\Buttons\\UI-DialogBox-Button-Highlight", "ADD")--"Interface\\Buttons\\UI-DialogBox-Button-Highlight"
		...
		decline:SetNormalTexture("Interface\\Buttons\\UI-DialogBox-Button-Up")--"Interface\\Buttons\\UI-DialogBox-Button-Up"
		decline:SetPushedTexture("Interface\\Buttons\\UI-DialogBox-Button-Down")--"Interface\\Buttons\\UI-DialogBox-Button-Down"
		decline:SetHighlightTexture("Interface\\Buttons\\UI-DialogBox-Button-Highlight", "ADD")--"Interface\\Buttons\\UI-DialogBox-Button-Highlight"
...
do
	...
	function DBM:LoadGUI()
		...
		if not IsAddOnLoaded("DBM-GUI") then
			local enabled = IsAddOnLoaded("DBM-GUI")
			if enabled == 0 then
				EnableAddOn("DBM-GUI")
			end
	...
	local function updateAllRoster(self)
		if IsInRaid() then
			if not inRaid then
				inRaid = true
				sendSync("H")
				SendAddonMessage("BigWigs", bwVersionQueryString:format(0, fakeBWHash), "RAID")
		...
		elseif IsInGroup() then
			if not inRaid then
				...
				inRaid = true
				sendSync("H")
				SendAddonMessage("BigWigs", bwVersionQueryString:format(0, fakeBWHash), "PARTY")
...
--MK
	...
	function DBM:GetNumRealPlayersInZone()
		...
		local total = 0
		local currentMapId = GetInstanceInfo("player")
		if IsInRaid() then
			for i = 1, GetNumGroupMembers() do
				local targetMapId = GetInstanceInfo("raid"..i)
			...
			for i = 1, GetNumPartyMembers() do
				local targetMapId = GetInstanceInfo("party"..i)
				if targetMapId == currentMapId then
					total = total + 1
				end
			end
...
function DBM:GetNumRealGroupMembers()
	...
	if IsInGroup() then
		for uId in self:GetGroupMembers() do
			local targetMapId = GetInstanceInfo(uId)
			if targetMapId == currentMapId then
...
do
	...
	function DBM:ZONE_CHANGED_NEW_AREA()
		...
		LastInstanceMapID = GetCurrentMapAreaID()
		for i, v in ipairs(self.AddOns) do
			if not IsAddOnLoaded(v.modId) and (checkEntry(v.zone, zoneName) or checkEntry(v.mapId, zoneId)) then
...
do
	...
	syncHandlers["C"] = function(sender, delay, mod, modRevision, startHp, dbmRevision, modHFRevision, event)
		...
		if LastInstanceType == "none" and (not UnitAffectingCombat("player") or #inCombat > 0) then--world boss
			...
			local playerZone = GetInstanceInfo("player")
			local senderZone = GetInstanceInfo(senderuId)
	...
	syncHandlers["PT"] = function(sender, timer, senderMapID, target)
		...
		if not dummyMod then
			...
			dummyMod.timer = dummyMod:NewTimer(20, "%s", 2457, nil, nil, 0, nil, nil, DBM.Options.DontPlayPTCountdown and 0 or 1, threshold)
		end
	...
	do
		...
		function breakTimerStart(self, timer, sender)
			if not dummyMod2 then
				...
				dummyMod2.text = dummyMod2:NewAnnounce("%s", 1, 58067)
				dummyMod2.timer = dummyMod2:NewTimer(20, "%s", 58067, nil, nil, 0, nil, nil, DBM.Options.DontPlayPTCountdown and 0 or 1, threshold)
			end
	...
	local function SendVersion(guild)
		...
		if DBM.Options.FakeBWVersion then
			SendAddonMessage("BigWigs", bwVersionResponseString:format(fakeBWVersion, fakeBWHash), IsInRaid() and "RAID" or "PARTY")
			return
		end
	...
	do
		...
		buttonaccept:SetNormalTexture("Interface\\Buttons\\UI-DialogBox-Button-Up")--"Interface\\Buttons\\UI-DialogBox-Button-Up"
		buttonaccept:SetPushedTexture("Interface\\Buttons\\UI-DialogBox-Button-Down")--"Interface\\Buttons\\UI-DialogBox-Button-Down"
		...
		buttondecline:SetNormalTexture("Interface\\Buttons\\UI-DialogBox-Button-Up")--"Interface\\Buttons\\UI-DialogBox-Button-Up"
		buttondecline:SetPushedTexture("Interface\\Buttons\\UI-DialogBox-Button-Down")--"Interface\\Buttons\\UI-DialogBox-Button-Down"
		...
		syncHandlers["WBE"] = function(sender, modId, realm, health, ver, name)
			...
			if realm == playerRealm and DBM.Options.WorldBossAlert then
	...
	function DBM:CHAT_MSG_ADDON(prefix, msg, channel, sender)
		if prefix == "D4C" and msg and (channel == "PARTY" or channel == "RAID" or channel == "WHISPER" or channel == "GUILD") then
			--sender = Ambiguate(sender, "none")
			handleSync(channel, sender, strsplit("\t", msg))
		elseif prefix == "BigWigs" and msg and (channel == "PARTY" or channel == "RAID") then
...
do
	...
	local function createFrame()
		...
		do
			...
			editBoxLeft:SetTexture("Interface\\ChatFrame\\UI-ChatInputBorder-Left")--"Interface\\ChatFrame\\UI-ChatInputBorder-Left"
			editBoxLeft:SetHeight(32)
			editBoxLeft:SetWidth(32)
			editBoxLeft:SetPoint("LEFT", -14, 0)
			editBoxLeft:SetTexCoord(0, 0.125, 0, 1)
			editBoxRight:SetTexture("Interface\\ChatFrame\\UI-ChatInputBorder-Right")--"Interface\\ChatFrame\\UI-ChatInputBorder-Right"
			editBoxRight:SetHeight(32)
			editBoxRight:SetWidth(32)
			editBoxRight:SetPoint("RIGHT", 6, 0)
			editBoxRight:SetTexCoord(0.875, 1, 0, 1)
			editBoxMiddle:SetTexture("Interface\\ChatFrame\\UI-ChatInputBorder-Right")--"Interface\\ChatFrame\\UI-ChatInputBorder-Right"
...
do
	local function onMonsterMessage(self, type, msg)
		...
		if dbmIsEnabled and combatInfo[GetRealZoneText()] then
			for _, v in ipairs(combatInfo[GetRealZoneText()]) do
		...
		if dbmIsEnabled and combatInfo[GetCurrentMapAreaID()] then
			for _, v in ipairs(combatInfo[GetCurrentMapAreaID()]) do
				if v.type == type and checkEntry(v.msgs, msg) or v.type == type .. "_regex" and checkExpressionList(v.msgs, msg) then
					self:StartCombat(v.mod, 0, "MONSTER_MESSAGE")
				elseif v.type == "combat_" .. type .. "find" and findEntry(v.msgs, msg) or v.type == "combat_" .. type and checkEntry(v.msgs, msg) then
					if IsInInstance() then--Indoor boss that uses both combat and message for combat, so in other words (such as hodir), don't require "target" of boss for yell like scanForCombat does for World Bosses
...
--MK
	function DBM:StartCombat(mod, delay, event, synced, syncedStartHp, syncedEvent)
		...
		if not checkEntry(inCombat, mod) then
			...
			if mod.lastWipeTime and GetTime() - mod.lastWipeTime < (event == "ENCOUNTER_START" and 3 or mod.reCombatTime2 or 20) then return end
			if event then
			...
			if event ~= "TIMER_RECOVERY" then
				...
				if self.Options.AlwaysShowSpeedKillTimer and mod.stats and not mod.ignoreBestkill and not mod.noStatistics then
					...
					if bestTime and bestTime > 0 then
						local speedTimer = mod:NewTimer(bestTime, L.SPEED_KILL_TIMER_TEXT, 54536, nil, false)
						speedTimer:Start()
					end
				end
				--update boss left
	...
	function DBM:UNIT_HEALTH(uId)
		...
		if dbmIsEnabled and InCombatLockdown() then
			...
			if self.Options.AFKHealthWarning and UnitIsUnit(uId, "player") and (health < 85) and UnitIsAFK("player") and self:AntiSpam(5, "AFK") then--You are afk and losing health, some griever is trying to kill you while you are afk/tabbed out.
...
function DBM:GetCurrentInstanceDifficulty()
	...
	local _, instanceType, difficulty, difficultyName, instanceGroupSize, playerDifficulty, isDynamicInstance = GetInstanceInfo()
	--	name, type, difficultyIndex, difficultyName, maxPlayers,
    --dynamicDifficulty, isDynamic, instanceMapId, lfgID = GetInstanceInfo()
	--奥妮克希亚的巢穴 raid 2 25人 25 0 false       黑石深渊 party 1  5 0 false 诺森德 none 1  0 0 false
	--冠军的试炼 party 2 5人（英雄） 5 0 false          [00:50:19] 十字军的试炼 raid 1 10人 10 0 false  冠军的试炼 party 2 5人（英雄） 5 0 false  冠军的试炼 party 1 5人 5 0 false   熔火之心 raid 1 40人 40 0 false
	--3 10人yx   4 25人yx   1 10人pt 2 25人pt  
	if (difficulty == 1 and instanceType == "none") then--draenor field returns 1, causing world boss mod bug.
		return "worldboss", L.RAID_INFO_WORLD_BOSS.." - ", difficulty, instanceGroupSize
...
--[[
function bossModPrototype:GetCurrentInstanceDifficulty() 
	local _, instanceType, difficulty, _, _, playerDifficulty, isDynamicInstance = GetInstanceInfo()
...
function DBM:HasMapRestrictions()
	...
	if not playerX or not playerY or not dims then
		return true
	end
	return false
end
...
--MK EJ_GetEncounterInfo(encounterID)
...
function DBM:GetSpellInfo(spellId)
	local returnedSpellId = spellId
	local name, rank, icon, castingTime, minRange, maxRange  = GetSpellInfo(spellId)
...
function DBM:UNIT_DIED(args)
	...
	----GUIDIsPlayer
	if self.Options.AFKHealthWarning and GUID == UnitGUID("player") and UnitIsAFK("player") and self:AntiSpam(5, "AFK") then--You are afk and losing health, some griever is trying to kill you while you are afk/tabbed out.
		--self:FlashClientIcon()
		self:PlaySound(8585)--So fire an alert sound to save yourself from this person's behavior.
...
do
	...
	local function getNumRealAlivePlayers()
		...
		local currentMapId = isInInstance and select(1, GetInstanceInfo("player")) or GetCurrentMapAreaID("player") or 0
		local currentMapName = C_Map.GetMapInfo(currentMapId) or L.UNKNOWN
		if IsInRaid() then
			for i = 1, GetNumGroupMembers() do
				if isInInstance and select(1, GetInstanceInfo("raid"..i)) == currentMapId or select(7, GetRaidRosterInfo(i)) == currentMapName then
					alive = alive + ((UnitIsDeadOrGhost("raid"..i) and 0) or 1)
				end
			end
		else
			alive = (UnitIsDeadOrGhost("player") and 0) or 1
			for i = 1, GetNumPartyMembers() do
				if isInInstance and select(1, GetInstanceInfo("party"..i)) == currentMapId or select(7, GetRaidRosterInfo(i)) == currentMapName then
					alive = alive + ((UnitIsDeadOrGhost("party"..i) and 0) or 1)
				end
			end
		end
...
do
	...
	function DBM:DemoMode()
		if not testMod then
			...
			testTimer2 = testMod:NewTimer(20, "%s ", "Interface\\Icons\\Spell_Shadow_ShadesOfDarkness", nil, nil, 1)
			testTimer3 = testMod:NewTimer(20, "%s  ", "Interface\\Icons\\Spell_Fire_SelfDestruct", nil, nil, 3, L.MAGIC_ICON, nil, 1, 4)--inlineIcon, keep, countdown, countdownMax
			testTimer4 = testMod:NewTimer(20, "%s   ", 48465, nil, nil, 4, L.INTERRUPT_ICON)
			testTimer5 = testMod:NewTimer(20, "%s    ", 48465, nil, nil, 2, L.HEALER_ICON, nil, 3, 4)--inlineIcon, keep, countdown, countdownMax
			testTimer6 = testMod:NewTimer(20, "%s     ", 48465, nil, nil, 5, L.TANK_ICON, nil, 2, 4)--inlineIcon, keep, countdown, countdownMax
			testTimer7 = testMod:NewTimer(20, "%s      ", 48465, nil, nil, 6)
			testTimer8 = testMod:NewTimer(20, "%s       ", 48465, nil, nil, 7)
		...
		testSpecialWarning2:Schedule(43, "恐惧")
...
do
	...
	function DBM:PLAY_MOVIE(id)
		if id and not neverFilter[id] then
			...
			if not isInstance or self.Options.MovieFilter2 == "Never" or self.Options.MovieFilter2 == "OnlyFight" then return end
...
do
	...
	function bossModPrototype:CanRemoveEnrage(uId)
		if specRoleTable[currentSpecID]["RemoveEnrage"] then
			return true
		else
			return false
		end
	...
	function bossModPrototype:IsMelee(uId)
		if uId then--This version includes monk healers as melee and tanks as melee
			...
			--Inspect throttle exists, so have to do it this way
			if class == "DRUID" then
				if UnitPowerMax(uId) < 11000 then
					return true
				end
			end
			return false
	...
	function bossModPrototype:IsManaUser()
		...
		if specRoleTable[currentSpecID]["ManaUser"] then
			return true
		else
			return false
		end
...
do
	...
	function bossModPrototype:NewSpecialWarning(text, optionDefault, optionName, optionVersion, runSound, hasVoice, difficulty, texture)
		...
		local seticon
		if texture then
			seticon = (type(texture) == "number" and select(3, GetSpellInfo(texture))) or texture
		end
		local obj = setmetatable(
			{
				text = self.localization.warnings[text],
...
do
					...
					if DBM.Options.BadTimerAlert or DBM.Options.DebugMode and DBM.Options.DebugLevel > 1 then
						...
						if bar then
							...
							if bar.timer > 0.2 then
								...
								local phaseText = self.mod.vb.phase and "MK" or ""
	...
	--local function newTimer(self, timerType, timer, spellId, timerText, optionDefault, optionName, texture, r, g, b)
		...
		elseif timerType == "adds" or timerType == "addscustom" then
			...
			if spellName then
				icon = type(texture) == "number" and select(3, GetSpellInfo(texture)) or texture or spellId and select(3, GetSpellInfo(spellId))
				--icon = type(texture) == "number" and select(3, GetSpellInfo(texture)) or tonumber(texture) or type(spellId) == "string" and select(4, DBM:EJ_GetSectionInfo(string.sub(spellId, 3))) ~= "" and select(4, DBM:EJ_GetSectionInfo(string.sub(spellId, 3))) or (type(spellId) == "number" and GetSpellTexture(spellId))
			else
				icon = nil
			end
		end
...
do
	...
	function bossModPrototype:NewCombatTimer(timer, text, barText, barIcon)
		...
		local bar = self:NewTimer(timer, barText or L.GENERIC_TIMER_COMBAT, barIcon or 2457, nil, "timer_combat", nil, nil, nil, 1, 5)
		local obj = setmetatable(
			{
				bar = bar,
				timer = timer,
...
function bossModPrototype:SetRevision(revision)
	...
	if not revision or revision == "20201225041541" then
...
do
	...
	function bossModPrototype:SetIcon(target, icon, timer)
		...
		--MK if uId and UnitIsUnit(uId, "player") and DBM:GetNumRealGroupMembers() < 2 then return end--Solo raid, no reason to put icon on yourself.
