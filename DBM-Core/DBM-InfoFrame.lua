...
local DBM = DBM
local L = DBM_CORE_L
local UnitClass, GetTime, GetPartyAssignment, UnitGroupRolesAssigned, GetRaidTargetIndex, UnitExists, UnitGetTotalAbsorbs, UnitName, UnitHealth, UnitPower, UnitPowerMax, UnitIsDeadOrGhost, UnitThreatSituation, GetInstanceInfo, UnitIsUnit, UIDropDownMenu_CreateInfo, UIDropDownMenu_AddButton = UnitClass, GetTime, GetPartyAssignment, UnitGroupRolesAssigned, GetRaidTargetIndex, UnitExists, UnitGetTotalAbsorbs, UnitName, UnitHealth, UnitPower, UnitPowerMax, UnitIsDeadOrGhost, UnitThreatSituation, GetInstanceInfo, UnitIsUnit, UIDropDownMenu_CreateInfo, UIDropDownMenu_AddButton
...
local function onUpdate(frame, table)
	...
	for i = 1, #sortedLines do
		...
		if friendlyEvents[currentEvent] then
			...
			if unitId and select(1, GetInstanceInfo(unitId)) == currentMapId then
...
function infoFrame:Show(modMaxLines, event, ...)
	...
	prevLines = 0
	currentMapId = select(1, GetInstanceInfo("player"))
