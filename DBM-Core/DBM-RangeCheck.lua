...
local function UnitDistanceSquared(uId)
	local x1, y1 = GetPlayerMapPosition("player")
	local x2, y2 = GetPlayerMapPosition(uId)
...
do
	...
	-- All ranges are tested and compared against .
	-- Example: Worgsaw has a tooltip of 6 but doesn't factor in hitboxes/etc. It doesn't return false until  of 8.
	local itemRanges = {
	...
	--		elseif IsItemInRange(32321, uId) then return 13
			...
			elseif IsItemInRange(uId, 4) then return 28
...
local function createRadarFrame()
	...
	player:SetTexture("Interface\\Minimap\\MinimapArrow.blp") -- "Interface\\Minimap\\MinimapArrow.blp"
	...
	for i = 1, 40 do
		...
		dot:SetSize(24, 24)
		dot:SetTexture("Interface\\Minimap\\PartyRaidBlips") -- "Interface\\Minimap\\PartyRaidBlips"
		dot:Hide()
...
----------------
do
	local UnitExists, UnitIsUnit, UnitIsDeadOrGhost, UnitIsConnected, UnitInPhase, GetPlayerFacing, UnitName, UnitClass, IsInRaid, GetRaidTargetIndex = UnitExists, UnitIsUnit, UnitIsDeadOrGhost, UnitIsConnected, UnitInPhase, GetPlayerFacing, UnitName, UnitClass, IsInRaid, GetRaidTargetIndex
	...
	function updateIcon()
		local numPlayers = IsInRaid() and GetNumRaidMembers() or GetNumPartyMembers()
		...
		for i = 1, activeDots do
			...
			if i <= numPlayers then
				...
				if icon and icon < 9 then
					dot.icon = icon
					dot:SetTexture("Interface\\TargetingFrame\\UI-RaidTargetingIcon_" .. icon) -- "Interface\\TargetingFrame\\UI-RaidTargetingIcon_" .. icon
	...
	function updateRangeFrame()
		...
		--MK想办法判断是不是一个地图?/run print(UnitIsVisible(UnitName(uId)))       local playerMapId = GetBestMapForUnit("player") or 0  
		...
		local totalpp = IsInRaid() and GetNumRaidMembers() or GetNumPartyMembers()
		...
		for i = 1, totalpp do
			...
			if UnitExists(uId) and UnitIsVisible(UnitName(uId)) and not UnitIsUnit(uId, "player") and not UnitIsDeadOrGhost(uId) and UnitIsConnected(uId) and (not filter or filter(uId)) then
				...
				if rEnabled then
					...
					dot.x = (x - playerX) * dims[1]
					dot.y = (y - playerY) * dims[2]
					dot.range = range
...
do
	...
	function getDistanceBetweenAll(checkrange)
		...
		for uId in DBM:GetGroupMembers() do
			if UnitExists(uId) and not UnitIsUnit(uId, "player") and not UnitIsDeadOrGhost(uId) and UnitIsConnected(uId) and UnitIsVisible(UnitName(uId)) then
	...
	function getDistanceBetween(uId, x, y)
		...
		if type(x) == "string" and UnitExists(x) then -- arguments: uId, uId2
			-- First attempt to avoid UnitPosition if any of args is player  should work
