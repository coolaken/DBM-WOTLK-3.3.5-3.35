local mod	= DBM:NewMod("ValkTwins", "DBM-Coliseum", 1)
...
local specWarnEmpoweredDarkness		= mod:NewSpecialWarningYou(67215, false)
local specWarnEmpoweredLight		= mod:NewSpecialWarningYou(67218, false)

local enrageTimer					= mod:NewBerserkTimer(360)
local timerSpecial					= mod:NewTimer(45, "TimerSpecialSpell", "Interface\\Icons\\INV_Enchant_EssenceMagicLarge", nil, nil, 2, nil, DBM_CORE_L.IMPORTANT_ICON)
local timerHeal						= mod:NewCastTimer(15, 65875, nil, nil, nil, 4, nil, DBM_CORE_L.INTERRUPT_ICON)
local timerLightTouch				= mod:NewTargetTimer(20, 67298, nil, nil, nil, 3, nil, DBM_CORE_L.DEADLY_ICON)
...
function mod:OnCombatStart(delay)
	...
	sndWOP:ScheduleVoice(40-delay, "twinsoon")
...
function mod:SpecialAbility(debuff)
	...
	sndWOP:ScheduleVoice(40, "twinsoon")
end
...
--[[
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg) --测试! 通过
	if msg == L.Test1 or msg:find(L.Test1) then
		SendChatMessage("1doudou", "SAY")
	elseif msg == L.Test2 or msg:find(L.Test2) then
		SendChatMessage("2doudou", "SAY")
	elseif msg == L.Test3 or msg:find(L.Test3) then
		SendChatMessage("3doudou", "SAY")
	elseif msg == L.Test4 or msg:find(L.Test4) then
		SendChatMessage("4doudou", "SAY")
	end
end
]]