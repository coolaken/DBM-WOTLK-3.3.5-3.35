local mod	= DBM:NewMod("Jaraxxus", "DBM-Coliseum", 1)
...
local warnFelFireball			= mod:NewCastAnnounce(66532, 2, 2.5)
local warnPortalSoon			= mod:NewSoonAnnounce(67900, 3)
local warnVolcanoSoon			= mod:NewSoonAnnounce(67901, 3)
local warnFlame					= mod:NewTargetNoFilterAnnounce(68123, 4)
local warnFlesh					= mod:NewTargetNoFilterAnnounce(66237, 5)
local warnNetherPower			= mod:NewAnnounce("WarnNetherPower", 4, 67009)

local specWarnFlame				= mod:NewSpecialWarningRun(67072, nil, nil, nil, 5, 3)
local specWarnFlesh				= mod:NewSpecialWarningYou(66237)
local specWarnKiss				= mod:NewSpecialWarningYou(67907, false)
local specWarnNetherPower		= mod:NewSpecialWarning("SpecWarnNetherPower", "MagicDispeller")
...
local SpecWarnFelFireballDispel	= mod:NewSpecialWarningDispel(66965, "RemoveMagic")

local timerCombatStart			= mod:NewCombatTimer(71.5)--rollplay for first pull
local enrageTimer				= mod:NewBerserkTimer(600)
local timerFlame 				= mod:NewTargetTimer(8, 68123, nil, nil, nil, 3, nil, DBM_CORE_L.HEALER_ICON)--There are 8 debuff Ids. Since we detect first to warn, use an 8sec timer to cover duration of trigger spell and damage debuff.
local timerFlameCD				= mod:NewCDTimer(30, 68125, nil, nil, nil, 3)
local timerNetherPowerCD		= mod:NewCDTimer(42, 67009, nil, nil, nil, 2, nil, DBM_CORE_L.DAMAGE_ICON)
local timerFlesh				= mod:NewTargetTimer(12, 67049, nil, "Healer", nil, 3, nil, DBM_CORE_L.HEALER_ICON)
local timerFleshCD				= mod:NewCDTimer(23, 67051, nil, nil, nil, 3) 
...
function mod:OnCombatStart(delay)
	...
	sndWOP:ScheduleVoice(15-delay, "doorsoon")
	warnPortalSoon:Schedule(15-delay)
	timerVolcanoCD:Start(80-delay)
	sndWOP:ScheduleVoice(75-delay, "volcanosoon")
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(67901, 67902, 67903, 66258) then		-- Infernal Volcano
		if mod:IsDifficulty("heroic10") or mod:IsDifficulty("heroic25") then
			sndWOP:Play("killvolcano")
		end
		timerVolcanoCD:Start()
		sndWOP:ScheduleVoice(115, "volcanosoon")
		warnVolcanoSoon:Schedule(110)

	elseif args:IsSpellID(67900, 67899, 67898, 66269) then		-- Nether Portal
		if mod:IsDifficulty("heroic10") or mod:IsDifficulty("heroic25") then
			sndWOP:Play("killdoor")
		end
		timerPortalCD:Start()
		sndWOP:ScheduleVoice(115, "doorsoon")
